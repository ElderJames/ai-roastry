...
public sealed class McpClientsFactory
{
    ...
    public McpClientsFactory(IDbContextFactory<LlmDbContext> dbContextFactory, ILoggerFactory loggerFactory)
    {
    ...
    public async Task<List<McpClientWrapper>> GetOrCreateClientsAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (mcpServerConfigs is null || !mcpServerConfigs.Any())
        {
            // 如果没有配置任何MCP服务器，返回空列表
        ...
        try
        {
            ...
            var mcpMcpServers = ASMcpServerConfig(mcpServerConfigs); 
            // Initialize clients
            var clients = await McpClientInitializer.InitializeClientsAsync(
                mcpMcpServers, _loggerFactory, cancellationToken);
    ...
    public async Task<McpClient?> GetMcpClientAsync(string serverId, CancellationToken cancellationToken = default)
    {
        ...
        {
            // 关键：在返回客户端之前进行健康检查和自动重连
            bool isHealthy = await clientWrapper.EnsureHealthyAsync(cancellationToken);

            if (!isHealthy)
            {
                _logger?.LogError($"Unable to ensure healthy connection for MCP Client '{serverId}'");
                return null; // 或者抛出异常，根据您的错误处理策略
            }
    ...
    public async Task AddClientAsync(string serverId, CancellationToken cancellationToken = default)
    {
        ...
        var mcpServerConfigs = await dbContext.McpServerConfigs.AsNoTracking().ToListAsync();
        var mcpMcpServers = ASMcpServerConfig(mcpServerConfigs);
        if (!mcpMcpServers.TryGetValue(serverId, out var serverConfig))
        {
    ...
    public Dictionary<string, McpServerConfigDto> ASMcpServerConfig(List<McpServerConfig> mcpServerConfig)
    {
