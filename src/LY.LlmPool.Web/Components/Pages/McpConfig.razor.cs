...
{
    public class McpConfigBase : ComponentBase
    {
        ...
        protected async Task OnAddServerFormFinish()
        {
            ...
            try
            {
                await SaveNewServer();
                addServerModalVisible = false;
                MessageService.Success($"Server '{newServerModel.ServerName}' has been added successfully.");
                await LoadServerDataAsync(); // This will also apply filters
            }
            catch (System.Exception ex)
            {
        ...
        {
            // Validate the model
            if (string.IsNullOrWhiteSpace(newServerModel.ServerName))
            ...
            var serverConfig = new McpServerConfigDto
            {
                ...
                Name = newServerModel.ServerName,
            ...
            await McpServerService.CreateServerAsync(serverConfig);
        }
        ...
        protected async Task OnEditServerFormFinish()
        {
            ...
            try
            {
                ...
                editServerModalVisible = false;
                MessageService.Success($"Server '{editServerModel.ServerName}' has been updated successfully.");
                await LoadServerDataAsync(); // This will also apply filters
            }
            catch (System.Exception ex)
            {
        ...
        {
            // Validate the model
            if (string.IsNullOrWhiteSpace(editServerModel.ServerName))
                throw new ArgumentException("Server Name is required");
            ...
            var serverConfig = new McpServerConfigDto
            {
                Id = editServerModel.ServerId,
                Name = editServerModel.ServerName,
            ...
            await McpServerService.UpdateServerAsync(serverConfig);
        }

        protected async Task FetchProto(string id)
        {
            try
            {
                _fetchingId = id;
                StateHasChanged();
                var (tools, prompts, resources) = await McpServerService.FetchAndCacheSchemaAsync(id);
                MessageService.Success($"已获取: 工具 {tools} 个，提示词 {prompts} 个,资源 {resources} 个");
                await LoadServerDataAsync();
            }
            catch (Exception ex)
            {
                MessageService.Error($"获取失败: {ex.Message}");
            }
            finally
            {
                _fetchingId = null;
